...
7. **Correction, found only after reading past the first viewport: the bounded high-contrast section already exists.** The first draft of this document (and decision 7's original text) proposed *adding* a dark `--color-sidebar` section to the landing page, reasoning from a Phase 0 baseline screenshot that only captured the first 900px. Scrolling further (`docs/evidence/baseline/01-landing-dark-section-1440-b.png`) shows `index.tsx` already has exactly this: a full `.site-plate` (`background: var(--color-sidebar)`) section with a real phone-frame product demo, bold white headline, and muted body lines - genuinely close in spirit to BridgeMind's "confident dark surface" quality, already shipped, already good. **No section needs to be added.** The real gap this exposed instead: that dark section currently uses zero colour accent (pure black/white/grey), and the *hero itself* - the actual first thing every visitor sees - is the flattest part of the whole page relative to what follows it. Phase 1's landing-page work is therefore narrower and different from the original proposal: a small `--color-mark-strong` accent on the hero's primary CTA, closing the energy gap between the hero and the page's own later sections, not a net-new dark section.
...
| Login / Signup / Auth-complete H1 | **Correction: already `.site-display`** (IBM Plex Serif 600, `clamp(1.7rem, 3.2vw, 2.55rem)`). The first draft of this document asserted these used plain `text-3xl` sans without reading `auth-request-form.tsx`/`auth.complete.tsx` first - they do not. These screens already have confident type, real named error states (link-expired, link-invalid, no-session, workspace-failed) and a non-dead-end sent-state with resend/correct-address. No H1 change needed here. | No change. |
| Onboarding H1 (`onboarding.tsx`) | `text-3xl font-semibold tracking-tight` (30px, sans) - **verified in source**, both stages | **Change**: adopt `.site-display`. This is the one auth-family screen genuinely behind the others. **Fallback**: if Phase 5 shows the bigger serif word sitting awkwardly above the form fields, bring field/label spacing up to match rather than reverting the heading. |
