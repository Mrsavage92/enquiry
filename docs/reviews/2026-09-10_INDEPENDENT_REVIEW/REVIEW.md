...
## Follow-up verification (same day, after the batch pass)

The batch findings above were produced by nine separate agents. Before building a plan on them, every P0 and each load-bearing P1 was re-checked by reading the code directly. Corrections to the first pass, with evidence:

**P0-3 (client store standing in for a confirmed server action) — CONFIRMED, and wider than reported.**

- `settings-page.tsx:12-13,49-53` binds "Pause outbound"/"Resume Enquiry" to the store's `pause`/`resume` (`prototype-store.ts:930-941`, pure `set()` calls). The three other pause controls (`trust-screen.tsx:145,154`, `account-menu.tsx:114-115`, `system-banners.tsx:48`) all go through `useLiveTrustMutations`, which does call `setBusinessPause`. Settings is the one that was missed. Trivial fix.
- `waiting-desk.tsx:224-234` and `:258-269` ("They accepted off-channel") are rendered in the ordinary live branch with no `demoMode` check, while the same file gates send and copy on `demoMode` at `:69` and `:96`. `acceptQuote` (`prototype-store.ts:1102-1186`) writes an inbound message with body "Yes that works. Please lock it in." attributed to the customer, an outbound "You're booked" message, flips lifecycle to BOOKED and inserts a booking - all client-side, nothing server-side. `conversation.tsx:106,127` renders these with the plain "Received"/"Sent" labels and no marker. The store imports nothing from `lib/server` (`prototype-store.ts:1-45`), so none of its actions can persist. `proposeRevision`, `releaseFollowUp`, `recordClientQuestion` and `markLost` in the same component share the wiring.
- `resolvePrice` (`prototype-store.ts:986-1019`) is hardcoded to `b.id === "northlight"` and two fixture knowledge ids. Live reachability is lower than the batch implied: `conflictChoices` (`situation.ts:131-136`) needs `conflictWith` on a knowledge item, `rows.ts:261` does map `conflict_with` from the database, but no server path writes that column today. The file's own comment (`situation-card.tsx:33-36`) says "it happens not to be reachable is not a guarantee". Grade held at P1, fix is to gate the control on `demoMode` until a server path exists.
- Business Brain `confirmLearning`/`dismissLearning`/`resolveConflict`/`setVoice` are store-only, but `teach` is never assigned a non-null value anywhere in non-test source (grep `teach: {` finds nothing), so the Teach dialog and learning list are unreachable live today. This is a **beta-gate gap** (`BETA_READINESS_GATE.md` §3 voice persistence, §7 "Teach Enquiry"), not a live defect. Moved to the plan as scope, not a bug.
- `live-mutations.ts:46-97`: each of the four trust mutations updates the store first and, on `writeThrough` failure, calls `onFailure` (a toast) but never reverts. The doc comment's "surfaced rather than swallowed" is true; "rollback" is absent. P1 stands.

**P0-1 (homepage auto-reply) — re-graded to P1 / product decision.** Mechanism confirmed (`prototype-store.ts:648-652`, `:1187-1222`; the fabricated message renders as "Received" with no marker). But it is inside a figure captioned "Interactive demo", and the product owner already recorded it as residual risk 2 in the CC1 report, "for separate authorisation". Calling it a P0 fabrication overstated it. What is genuinely missing is a marker on the simulated message itself (`PUBLIC_TRAFFIC_GATE.md` §4: "demo/video send language makes the owner/manual action boundary truthful"), plus two hygiene gaps: `hydrateFromServer` does not clear `replyTimer` (only `reset()` does, `:305-315`), and `receiveClientReply` never re-checks `demoMode` when the timer fires.

**P0-2 (real tenant data in the homepage widget) — re-graded to P1.** Reachable: `account-menu.tsx:85` and `more-sheet.tsx:74` link a signed-in operator to `/` by client-side navigation; `live-phone.tsx:18` then falls back to `s.enquiries[0]`, the tenant's own first real enquiry, and `PhoneDesk` renders it with `demoMode:false`, so its actions hit the real server. It is the operator's own data on their own screen, not cross-tenant, so P1 rather than P0. Still a must-fix before public traffic: a marketing widget must never bind to live data.

**P0-4 (`ruleFingerprint` collision) — re-graded to P2.** Code confirmed (`business-rule.ts:155-169`, `.join("|")` on trimmed, unrestricted strings; `parseBusinessRule` at `:62-99` only trims). But rule payloads come from the owner's pricing form, and the interpreter's output schema carries no rule or price fields (per the lib-auth-interpret batch), so a `|` in `unit` or `quantityField` needs an owner to type one. Low likelihood, high impact, one-line fix (`JSON.stringify` the parts). Still worth a test.

**P1 (first-match fact selection in `compilePrice`) — re-graded to P2, neutralised upstream on the live path.** `decision-apply.ts:99` loads facts `where superseded = false` before deciding, and `enquiry-actions.ts:454-459` supersedes the earlier answer on every re-answer, so the server never hands `compilePrice` a stale fact. `decideEnquiry` has no client caller with real facts (`add-enquiry.tsx:40` passes `facts: []`). `toCompilerFacts` (`decide.ts:72-78`) should still drop `superseded` facts so the domain function is safe by construction.

**P1 (auth rate limiting) — re-scoped to an owner configuration task.** `client.ts:133-168` calls Supabase Auth directly from the browser, so no application-level limiter can sit in front of it. The control that exists is Supabase's Auth rate-limit settings; the action is to check them, not to write code.

**Confirmed unchanged:** `trust-screen.tsx:189,325` (`?? businesses[0]` with no `!business` guard, unlike `:37`); `protect.server.ts:18` trusts the first `x-forwarded-for` entry ahead of `x-real-ip`; `live-mutations.ts` no-rollback; no 404 route.

Net after verification: **0 P0s in the strict sense, 4 must-fix P1 clusters before public traffic or beta, and CC1 still not ready to sign off** - the reason is unchanged: the same defect class CC1 closed for one control is live in the others.
