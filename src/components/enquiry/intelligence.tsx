        ...
        <Panel title="Why this">
          ...
          {compact ? (
            ...
            <p className="mt-4 text-xs text-stone">
              Decision Engine {enquiry.decision.evaluators.length} evaluators · High/Medium/Low is
              explanatory, not an automation switch.
            </p>
          )}
        </Panel>
      </Dialog>
...
function groundedSummary(enquiry: Enquiry): string | null {
  ...
  const parts = ids
    ...
    .filter(
      (f): f is EnquiryFact =>
        Boolean(f?.displayValue) && f!.displayValue !== "-" && f!.displayValue !== "Not given",
    )
