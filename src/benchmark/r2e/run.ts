...
async function main(): Promise<void> {
  ...
  const report = [
    ...
    '- `case-11-conflicting-rule` demonstrates the mechanism that exists today for a conflicting rule (marking it "Needs review" removes it from `activeRules()` entirely, so nothing stale is ever used silently). It does not exercise two simultaneously-Active rules for the same service. Since CC1, `selectRule` is a thin wrapper over `matchRule`, which orders candidates by content fingerprint and returns `ambiguous` (never a first-match price) when two distinct Active rules cover one service; that path is covered by `price-safety.test.ts` S02/S03, not by this benchmark, so it is still not claimed here.',
    "",
  ].join("\n");
