...
test("a business with no rules escalates and says why - it does not invent a price", () => {
  ...
  assert.match(d.explanation, /not added any prices yet/);
  // The next step is somewhere the owner can act, not a dead end.
  assert.equal(d.setup, "add_prices");
});

test("with prices but no service named, the next step is to say which service", () => {
  const d = decideEnquiry({ knowledge: [knowledge("Active")] }, { serviceLabel: "", facts: [] });
  assert.equal(d.setup, "choose_service");
  assert.doesNotMatch(d.explanation, /""/);
});
...
test("a service nothing covers escalates by name", () => {
  ...
  assert.equal(d.setup, "add_price");
});
