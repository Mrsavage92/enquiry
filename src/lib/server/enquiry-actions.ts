...
export const answerEnquiryFact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { answerFactForUser } = await import("@/lib/repo/answer-fact-core");
    const sql = await getSql();
    const result = await answerFactForUser(sql, withTransaction, context.userId, data);
    await recordAudit(result.businessId, {
      actor: context.userId,
      summary: `${data.field} confirmed as "${result.value}"`,
      objectType: "enquiry",
    ...
    return {
      ...
      explanation: result.decision.explanation,
      revision: result.revision,
      // The total now on the quote, so the card can say "Priced: $570".
      amountMinor:
        result.decision.action === "SEND_QUOTE" && result.decision.price.kind === "EXACT"
          ? result.decision.price.amountMinor
          : null,
    };
  });
